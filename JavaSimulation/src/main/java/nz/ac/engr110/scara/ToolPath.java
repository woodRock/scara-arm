...
public class ToolPath {
    ...
    private static final int LOW = 1100;
    private int nSteps;
    private final ArrayList<Double> theta1Vector;
    private final ArrayList<Double> theta2Vector;
    private final ArrayList<Integer> penVector;
    private final ArrayList<Integer> pwm1Vector;
    private final ArrayList<Integer> pwm2Vector;
    private final ArrayList<Integer> pwm3Vector;
    ...
    public void convertDrawingToAngles(Drawing drawing, Arm arm) {
        for (int i = 0; i < drawing.getDrawingSize() - 1; i++) {
            ...
            this.nSteps = (int) distance(p0.getX(), p1.getX(), p0.getY(), p1.getY());
            for (int j = 0; j < this.nSteps; j++) {
                double x = p0.getX() + j * (p1.getX() - p0.getX()) / this.nSteps;
                double y = p0.getY() + j * (p1.getY() - p0.getY()) / this.nSteps;
    ...
    private boolean isInvalidFile(String file) {
        return file == null;
    }
